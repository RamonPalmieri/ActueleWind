...
def get_wind_data(station_code):
    uri = f"https://actuelewind.nl/api/getSpotDetail.php?id={station_code}"

    try:
        response = requests.get(uri, headers={'User-Agent': 'ActueleWind-Script/1.0'})
        response.raise_for_status()

        data = response.json()

        station_name = data['info']['stationnaam']
        windrichting_van = data['info']['windrichtingVan']
        windrichting_tot = data['info']['windrichtingTot']
